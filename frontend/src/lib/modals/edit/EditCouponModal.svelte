<script>
    import { PathUpdateCoupon } from "$lib/api/paths";
...
loading = true
        ...
        formData.set("onhold",$couponStore.onhold)
        
        let res = await fetch(PathUpdateCoupon($couponStore.id),{
            headers:{
        ...
        if(res.status == "success") {
            close.click()
            let text = `Edited ${$couponStore.name} ` 
            toast(text,"success")
    ...
    <div class="modal  fade" id="editCouponModal" tabindex="-1" aria-labelledby="exampleModalgridLabel" aria-modal="true"  on:hidden.bs.modal={reset}>
                ...
                <form  on:submit|preventDefault={save} bind:this={form} class:d-none={loading}>
                        <div class="row g-3">
                                ...
                                <div>
                                    ...
                                    <input type="text" class="form-control" id="couponName" name="name" placeholder="Enter Coupon name" bind:value={$couponStore.name}>
                                    {#if errors?.name}
                                ...
                                <div>
                                    ...
                                    <input type="text" class="form-control" id="couponName" name="code" placeholder="Enter Coupon code"bind:value={$couponStore.code}>
                                    {#if errors?.code}
                                ...
                                <div>
                                    ...
                                    <input type="text" class="form-control" id="couponName" name="max" placeholder="Enter Coupon max" bind:value={$couponStore.max}>
                                    {#if errors?.max}
                                ...
                                <div class="row ps-3 g-3">
                                    ...
                                    <div class="form-check form-switch col" >
                                        <input class="form-check-input" type="checkbox" role="switch" id="SwitchCheck1" bind:checked={$couponStore.onhold}>
