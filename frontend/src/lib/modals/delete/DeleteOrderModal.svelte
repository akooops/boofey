<script>
    import {PathDelOrder} from "$lib/api/paths.js"
...
loading = true
        ...
        let res = await fetch(PathDelOrder($orderStore.id),{
            headers:{
        ...
        if(res.status == "success"){
            close.click()
            let text = `Deleted #${$orderStore.id}` 
            toast(text,"success")
            invalidate("orders:refresh")
        }
...
<div id="deleteOrderModal" class="modal fade" tabindex="-1" aria-hidden="true" style="display: none;">
