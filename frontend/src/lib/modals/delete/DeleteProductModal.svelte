<script>
    import {PathDelProduct} from "$lib/api/paths.js"
...
loading = true
        ...
        let res = await fetch(PathDelProduct($productStore.id),{
            headers:{
        ...
        if(res.status == "success"){
            close.click()
            let text = `Deleted ${$productStore.name} #${$productStore.id}` 
            toast(text,"success")
...
<div id="deleteProductModal" class="modal fade" tabindex="-1" aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-dialog-centered">
        ...
        <div class="text-end">
                ...
                <div class="mt-2" class:d-none={loading}>
                    ...
                    <p class="text-muted fs-15 mb-4">Are you Sure You want to Delete <span class="text-primary">{`${$productStore.name} #${$productStore.id}`}</span> ?</p>
