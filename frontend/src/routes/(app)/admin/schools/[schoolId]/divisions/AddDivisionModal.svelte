<script>
    import { PathAddDivision } from "$lib/api/paths";
...
loading = true
        ...
        let res = await fetch(PathAddDivision(schoolId),{
            headers:{
        ...
        if(res.status == "success") {
            close.click()
            let text = `Added ${divisionName} as a new division` 
            toast(text,"success")
    ...
    <div class="modal  fade" id="addDivisionModal" tabindex="-1" aria-labelledby="exampleModalgridLabel" aria-modal="true"  on:hidden.bs.modal={reset}>
                ...
                <form  on:submit|preventDefault={save} bind:this={form} class:d-none={loading}>
                        <div class="row g-3">
                                ...
                                <div>
                                    ...
                                    <input type="text" class="form-control" id="divisionName" name="name" placeholder="Enter Division name" bind:value={divisionName}>
                                    {#if errors?.name}
                                ...
                                <div>
                                    ...
                                    <input type="text" class="form-control" id="divisionName" name="name_ar" dir="rtl" placeholder="ادخل اسم القسم">
                                    {#if errors?.name_ar}
