<script>
    import {PathDelDivision} from "$lib/api/paths.js"
...
loading = true
        ...
        let res = await fetch(PathDelDivision($divisionStore.id),{
            headers:{
        ...
        if(res.status == "success"){
            close.click()
            let text = `Deleted ${$divisionStore.name} #${$divisionStore.id}` 
            toast(text,"success")
...
<div id="deleteDivisionModal" class="modal fade" tabindex="-1" aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-dialog-centered">
        ...
        <div class="text-end">
                ...
                <div class="mt-2" class:d-none={loading}>
                    ...
                    <p class="text-muted fs-15 mb-4">Are you Sure You want to Delete <span class="text-primary">{`${$divisionStore.name} #${$divisionStore.id}`}</span> ?</p>
