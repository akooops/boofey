<script>
    ...
    import { PathUpdateOrder } from "$lib/api/paths";
    ...
    $: {
        subtotal = productsList.reduce((total, product) => {
            return total + product.product.currentPrice * product.qty;
        }, 0);
    }
    $: {
    ...
    async function save(){
        ...
        let res = await fetch(PathUpdateOrder(id),{
            headers:{
